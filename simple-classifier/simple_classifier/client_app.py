...
import syft_flwr # **ADDED import syft_flwr**
...
from simple_classifier.task import (
    get_model,
    get_model_params,
    #load_data,    #**Changed**
    load_syftbox_dataset as load_data,  # **CHANGED to use syftbox dataset**
    set_initial_params,
    set_model_params,
)
...
def evaluate(msg: Message, context: Context):
    ...
    partition_id = context.node_config["partition-id"]
    num_partitions = context.node_config["num-partitions"]
    _, X_test, _, y_test = load_data() #* Removed (partition_id, num_partitions)*
